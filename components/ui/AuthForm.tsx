...
const AuthForm = ({type}: {type: string}) => {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      //Sign with appwrite & alseo create plain link token
      if(type === 'Sign-up') {
    ...
    <section className='auth-form'>
        <header className='flex flex-col gap-5 md:gap-8'>
            <Link href='/' className='cursor-pointer flex items-center gap-1'>
                ...
                alt='Prabhu logo'
               />
               <h1 className='text-26 font-ibm-plex-serif font-bold text-black-1'>PRABHU</h1>
            </Link>
                 ...
                 <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  {type === 'Sign-up' && (
                    ...
                    <CustomInput
                       ...
                       <div className='flex gap-4'>
                       <CustomInput
                       control ={form.control} name ='state' label="State" placeholder='Example:Kathmandu' />
                       <CustomInput
                       control ={form.control} name ='postalCode' label="Postal Code" placeholder='Example:977' />
                       </div>
                       <div className='flex gap-4'>
                       <CustomInput
